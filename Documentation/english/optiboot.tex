...
\item {Supports the app do\_spm, which was included with the original optiboot version 8.
	This is located 2 bytes behind the boot loader start address.}

\item {Installing the optiboot bootloader at a respective AVR can be done with the program \textbf{avrdude} and
	a connected ISP-programmer.
	To initiate this task only a additional ISP=1 is required at the \textbf{make} call.
	With a ISP=2 at the \textbf{make} call you can operate a verify run of \textbf{avrdude}.
...
\item { Supports the do\_spm app, which was included with the original Optiboot version 8.
        It is located 2 bytes after the bootloader start address.}	
...
in the hfuse or efuse byte, which does not \textbf{make} the correction easier.
After the required corrections the \textbf{avrdude} program is called with
...
If the \textbf{avrdude} call returns with an error, the script give some common hints
...
You can select some parameters also with the \textbf{make} call or by setting
...
This replacement of JMP instruction in the vector table by RJMP is now respected
by the optiboot program.
The optiboot program checks, if the vector 0 has a RJMP instruction.
in this case the RJMP is corrected for the shifted new vector place.
A JMP instruction can be moved ito the new place without any correction.
The original content of the shifted vector place is saved in the last four bytes of EEprom memory.
With that features a verify of program data is anytime possible,
because the content of original vectors are restored in serial data.
But you can see the modified vector content, if you read back the Flash memory with
other programming interfaces like ISP.
...
The communication program like \textbf{avrdude} at the external side is usually unprepared for this condition.
...
With this circuit you can use an unchanged transfer program like \textbf{avrdude} for example.
...
A workaround would be to let the transmitter of the data (\textbf{avrdude}) send all data with 2 stop bits.
Because this is not intended by the \textbf{avrdude} version 6.3,
...
The PC side with \textbf{avrdude} version 6.3 could send data with two stop bits.
So I had pached the sources for \textbf{avrdude} to add this feature with the additional option -S.
But unfortunately this feature is missing with \textbf{avrdude}.
...
\begin{Verbatim}[frame=single,rulecolor=\color{red}]
make atmega328p LED_START_FLASHES=0 BAUD_RATE=52
\end{Verbatim}
\begin{Verbatim}[frame=single,rulecolor=\color{blue},fontsize=\footnotesize]
...
iThis fulfills the essential requirements for systematic tests of the interaction between \textbf{avrdude},
...
For this, downloads with \textbf{avrdude} should be repeated with different clock frequencies,
...
A total of 1640 downloads of 10 kByte with \textbf{avrdude} was the base for each table entry.
A special modified \textbf{avrdude} program was used for these tests,
which can select a serial output with two stop bits with the new parameter -S.
The differences of the \textbf{avrdude} sources are notices in the file ee\_test/avrdude.diff.
...
For the tests downloads of 10 kByte each with \textbf{avrdude} are repested 40 times for each frequency.
...
respective clock rate (column 1) of 40 download attempts of 10 kByte with \textbf{avrdude}.
...
40 downloads of 10 kByte each with the modified \textbf{avrdude} (2 stop bits) are tried for each frequency.
...
Unfortunately the standard \textbf{avrdude} with version 6.3 does not allow the use of a second stop bit.
You can try to patch the \textbf{avrdude} version 6.3 sources with the diff documentation in the
...
You should try the configurations of optiboot to fit in 512 byte flash with \textbf{make} calls without the ISP option or with
...
with every \textbf{make} call that specifies a target processor.\\
...
from \textbf{avrdude} with the use of a second stop bit for the transmitted serial data.
It is a little surprising that this has not been noticed so far.
Otherwise the second stop bit should at least be selectable as an option from \textbf{avrdude}.\\
...
In addition, a \textbf{make} call with the desired target processor as a parameter is sufficient
