...
Wenn der Typ für Sie nicht richtig ist, konnen sie das beim \textbf{make} Aufruf ändern wie:
\begin{verbatim}
...
in das optiboot verzeichnis und tippen sie die \textbf{make} Kommandos ab.
...
Trotz des einfachen Aufrufes würde diese Konfiguration für gängige Platinen wir Arduino UNO oder Arduino Nano passen.
...
avr-gcc -g -Wall -Os -fno-split-wide-types -mrelax -mmcu=atmega328  -fno-diagnostics-show-caret 
 ...
 -DUART_-DSUPPORT_EEPROM=0RX=pD0 -DUART_TX=pD1 -DF_CPU=16000000 -DHFUSE=hexDE -DLFUSE=hexFF -DBOOT_PAGE_LEN=512 
...
Noch einmal prüfen, ob da was schief gelaufen ist. Beim avr-gcc Aufruf ist aber an letzter Stelle das Programm optiboot.c
...
der Hinweis \inquotes{ und EEprom Unterstützung}. Richtig, in der Parameterliste beim avr-gcc Aufruf
...
angepasst werden. Es ist aber jetzt auch verständlich, warum bei der Standard-Konfiguration für
...
für den Kompiler avr-gcc aus der automatisch gesetzten Option \inquotes{-DSOFT\_UART=1} erkennen sowie beim \inquotes{BAUD RATE CHECK} aus
...
Diese Beispiele sollen zeigen, was die vom \textbf{make} Aufruf alles über den Ziel-Prozessor wissen.
...
Schritt generell bei langsamer Bitrate (\textbf{avrdude -B 200}) durchgeführt.
...
Das hat \textbf{avrdude} festgestellt, also läuft die Kommunikation! 
...
hoch bis der Bootloader wieder erreicht wird und die LED wieder einschaltet.
