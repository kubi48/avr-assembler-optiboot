...
auf den Zielprozessor zu schreiben.
...
Eingabe von Parametern beim make Aufruf geändert werden können.
Die Fahigkeiten des optiboot Bootloaders sind im Kapitel \ref{sec:optiboot} ab Seite \pageref{sec:optiboot}
beschrieben.
...
Parameter beim make Aufruf angegeben werden.
...
Bootspeicherseiten sowie die Größe des Bootloaders angegeben.

\section{Exotische Beispiele für die optiboot Erstellung}
Wenn wir nun den Aufruf mit einem anderen Zielprozessor wiederholen, sieht das Ergebnis völlig
anders aus:
...
aud Pin PD0 und PD1 für den Bootloader benutzt. Das Fehlen einer seriellen Schnittstelle kann man in der Optionsliste
für den Kompiler avr-gcc aus der automatisch gesetzten Option -DSOFT\_UART=1 erkennen sowie beim BAUD RATE CHECK aus
dem Wort SoftUART\_Real. Die angegebene Differenz zur gewünschten Baudrate von 115000 sollte einen aber nicht
...
Deswegeb ist das Bootloaderprogramm deutlich größer und benutzt 10 Flashspeicher-Seiten mit jeweils 64 Bytes.
...
Diese Beispiele sollen zeigen, was die vom make Aufruf alles über den Ziel-Prozessor wissen.
Wenn man über die Eingabe von Parametern die Standard Einstellung des Bootloaders verändern will, sollte man
aufmerksam das Bildschirm-Protokoll lesen, ob das Ergebnis so gewünscht wird.
...
Verfahren ausgewählt, die im Unterkapitel \ref{sec:autobaud} auf Seite \pageref{sec:autobaud} beschieben sind. Aus den Untersuchungen ist die Angabe einer 32 sinnvoll:
...
Das sollte doch reichen und brauch sicher nicht viel Programmplatz, also:
...
ISP-Schnittstelle geben. Das ist idealerweise ein 6-poliger oder ein 10-poliger Wannenstecker wie er
auf Abbildung \ref{fig:ISP} auf Seite \pageref{fig:ISP} dargestellt ist.
Wir benötigen ebenfalls einen ISP-Programmer, der idealerweise einen gleichen Wannenstecker haben sollte.
Einen Flachbandkabel mit passenden Steckbuchsen wird auch benötigt. Ein Flachbandkabel wird oft mit dem
...
der Takt noch deutlich geringer ist als beim make Aufruf für den Bootloader spezifiziert, wird dieser
Schritt generell bei langsamer Bitrate (avrdude -B 200) durchgeführt.
Dieser Schritt läuft also bei einem neuen Atmel Prozessor. Wenn der Atmel Prozessor aber schon
auf den Quarzbetrieb eingestellt ist, läuft auch schon der erste iProgrammier-Schritt nicht, da jeglicher Takt
für den Prozessor ohne Quarz fehlt. Aus dem gleichen Grund können für alle Zustände des Atmel Prozessors
die weiteren Programmier-Schritte nicht ohne Quarz laufen.
Abhilfe für dieses Problem schafft ein zusaätzlich am Programmier-Sockel angebrachter Quarz.
Außer dem ersten Programmierschritt (setzen der Fuses) wird die Bitrate (avrdude -B) 
...
\end{verbatim}
\normalsize
Dies Beispiel-Protokoll stammt von einem älteren Diamex AVR-Prog Programmer und
ist mit dmesg -t (ohne Zeit) erzeugt. Sie sollten aber die Option -T verwenden,
um die Zeit des Entrage kontrollieren zu können. Dieser Programmer ist
zum Atmel AVRISP mkII kompatibel und ist als Programmer voreingestellt (ISPPORT=usb , ISPTOOL=avrisp2).
Für diesen Programmer braucht an den make Aufruf nur der Parameter ISP=1 angehängt zu werden.
...
\end{verbatim}
\normalsize
Da ist doch etwas schiefgelaufen! Das Programm avrdude hat mit einem Fehler abgebrochen,
der Grund ist an der avrdude Fehlermeldung zu erkennen:
\begin{verbatim}
avrdude error: expected signature for ATmega328 is 1E 95 14
        double check chip or use -F to override this check
\end{verbatim}
Das Programm avrdude erwartet eine andere Signatur des Mikrocontrollers.
Die nachfolgenden Ratschläge sind uninteressant. Das Programm avrdude
kehrt bei jedem Fehler mit Rückgabewert 1 zurück, der aufrufende Script
geht aber von einem Kommunikationsproblem des Programm avrdude aus.
...
Das hat avrdude festgestellt, also läuft die Kommunikation! 
...
Der Vorteil dieser Art von Programmern ist, daß sie vom Programm avrdude gefunden werden,
wenn als Schnittstelle (-P) usb angegeben wird. 
...
Da die Spezialchips relativ teuer sind, wird aber meistens eine Emulation mit einem Mikrocontroller verbaut.
...
dmesg -t | tail -20
\end{verbatim}
Hier wurde statt -T ein -t verwendet, da die Zeit beim gedruckten Beispiel uninteressant ist.
Sie sollten aber dmesg -T verwenden um die Zeit des Eintrags kontrollieren zu können.
Kurz nach dem Einstecken eines Diamex EXA-PROG Programmers sieht man so folgendes:
\begin{verbatim}
...
...
für avrdude richtig einzustellen. Der Name der Schnittstelle wird beim avrdude Programm mit dem
Parameter -P angegeben. Beim make Aufruf wird die Schnittstelle mit dem Parameter ISPPORT angegeben.
Das Protokoll für den ISP-Programmer wird beim Programm avrdude mit dem Parameter -c angegeben,
beim make Aufruf heißt der entsprechende Parameter ISPTOOL.
...
\end{verbatim}
\normalsize
Dummerweise weiß das Betriebssystem nichts über den and die serielle Schnittstelle
angeschlossenen ISP-Programmer.
Für die Einstellung des Parameters ISPTOOL (Option -c für avrdude) sollte das Handbuch des Programmers oder
...
hoffentlich Hinweise. Das Programm avrdude unterstützt viele Programmer.
wenn Sie avrdude nur mit der Spezifikation des Zielprozessors -p atmega328p aufrufen, werden 
...
Manchmal gibt der Name der von avrdude angezeigten Programmer Einstellungen einen Hinweis auf die Eignung
