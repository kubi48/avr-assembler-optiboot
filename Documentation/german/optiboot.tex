...
\section*{}
Der \textbf{optiboot} Bootloader wurde in der Sprache C von Peter Knight und
...
Dabei möchte ich betonen, daß ich den \textbf{optiboot} Bootloader nicht
...
die für die \textbf{optiboot} Tests verwendet wurden. 

\begin{figure}[H]
\centering
\includegraphics[width=9cm]{../FIG/Samples_AVR_boot.jpg}
\caption{Einige Testplatinen und AVRs für den \textbf{optiboot} Test}
...
Ein Empfang von Programmdaten für \textbf{optiboot} ist derzeit nur über eine serielle Schnittstelle möglich.\\

Hier sind einige der Fähigkeiten des \textbf{optiboot} Programms aufgelistet:
...
\item {Kann eine automatische Baudraten-Anpassung vornehmen.
	...
	Wenn der so konfigurierte \textbf{optiboot} noch in 512 Byte Flash passen soll,
...
\item {Kann auch für AVRs ohne Bootloader Unterstützung benutzt werden.\\
	...
	Das \textbf{optiboot} Programm wird dabei größer, kann aber dann auf jeder
...
\item {Das Laden des \textbf{optiboot} Programms in den jeweiligen AVR Prozessor kann automatisch mit dem
	Programm avrdude und einem angeschlossenem ISP-Programmer erfolgen.
	Dazu ist nur die zusätzliche Eingabe von ISP=1 beim \textbf{make} Aufruf erforderlich.
...
\item { Unterstützt die App do\_spm, welche mit der Original optiboot Version 8 eingebaut wurde.
	Die befindet sich 2 Byte hinter der Bootloader Startaddresse.}
...
\begin{itemize}
	...
	\item {Für die Erzeugung des \textbf{optiboot} Bootloaders können wahlweise auch angepaßte C-Quellen
	benutzt werden. Die meisten Funktionen sind auch hier möglich.
	Natürlich wird das erzeugte \textbf{optiboot} Programm größer!}

	\item {Der Umfang der Bildschirmausgabe bei der Erzeugung von \textbf{optiboot} kann mit der Systemvariablen
...
\item {Die Erzeugung des \textbf{optiboot} Programms wird in einer Bash Script Datei gesteuert, wobei die
...
\item {Die Erzeugung des \textbf{optiboot} Programms läuft unter Linux mit installierten avr Paketen.
...
\textbf{optiboot} Hex-Datei in Intel Format erzeugt wird.
...
um den AVR mit dem \textbf{optiboot} Bootloader zu beschreiben und alle Fuses zu setzen.
...
\item [def\_colors.sh] wird von verschiedenen Scripts eingefügt, um die Terminalausgabe
mit Farben zu versehen.

\item [show\_help2.sh] gibt Hilfetext für optiboot aus.

\item [show\_help3.sh] gibt weiteren Hilfetext für optiboot aus.
...
\textbf{make} Programms als Parameter oder auch als Umgebungsvariable der Shell
...
Mit den Optionen wird die Eigenschaft des \textbf{optiboot} Bootloaders eingestellt.
...
\begin{table}[H]
 ...
 TIMEOUT\_MS       & TIMEOUT\_      & Diese Option gibt eine Zeitschranke in Millisekunden \\
    ...
    \hline
    NO\_APP\_SPM   & NO\_APP\_SPM=1 & Setzt die Nutzerapplikation do\_spm außer Funktion wenn 1.\\
                   &                & NO\_APP\_SPM=2 bewirkt ein völliges Entfernen \\
                   &                & der Zusatzfunktion und spart 4 Byte Flash. \\
    \hline
    \end{tabular}
  \end{center}
...
PAGE\_ERASE        & PAGE\_ERASE=1  & Daten über die serielle Schnittstelle empfangen sind. \\
		   ...
		   &                & Dafür spart man beim \textbf{optiboot} Bootloader etwa 14 Bytes \\
...
eine Möglichkeit vorgesehen, trotzdem \textbf{optiboot} zu benutzen. 
...
Dies wird jetzt auch von \textbf{optiboot} berücksichtigt.
Das Optiboot Programm prüft, ob im Vektor 0 ein rjmp Befehl steht,
der dann für das Umplazieren auf den neuen Vektor korrigiert werden muß.
Das Umplazieren eines JMP Befehls erfordert keine Korrektur.
Für das Sichern des überschriebenen ,,Ersatz''-Vektors werden die vier letzten Bytes des EEprom-Speichers benutzt.
Eine Überprüfung des geschriebenen Anwender-Programms ist so möglich,
da die zurückgelesenen Interrupt-Vektor Inhalte vom Bootloader in den seriellen Daten korrigiert werden.
Der wahre Inhalt der Interrupt-Vectoren kann nur zurückgelesen werden, wenn andere
Programmier-Schnittstellen wie ISP benutzt werden.
...
Für die \textbf{optiboot} Applikation wird das von der Datei optiboot.S gemacht.
...
Bei der hier bei \textbf{optiboot} gewählten Software-Lösung ist ohnehin nur ein Halb-Duplex Betrieb
...
Normalerweise wird in \textbf{optiboot} bei der seriellen Ausgabe ein verlängertes Stoppbit ausgegeben.
...
Abschalten läßt sich die zwei Stoppbit Ausgabe bei \textbf{optiboot} übrigens mit der Option TWO\_STOP\_BITS=0.
...
Das zweite Beispiel zeigt die Erzeugung von \textbf{optiboot} für den gleichen Prozessor mit einer Software-Lösung
...
Schreibschutz-Pin in der \textbf{optiboot} Software vorgesehen. Diese Zusatzfunktion von \textbf{optiboot}
...
Taktfrequenz in \textbf{optiboot} eingebaut werden. Die Verbesserung besteht aus einer Zurücksetzung des
...
Erwähnen möchte ich hier, daß frühere Versionen der \textbf{optiboot} Autobaud Messung mit
...
Für diese Untersuchung wurde die \textbf{optiboot} Baudrate fest auf 115200 
...
Mit der Frequenzänderung ändert sich dann die \textbf{optiboot} Baudrate proportional mit.
...
Software-UART Lösung von \textbf{optiboot} untersucht. Die Software benutzt beim Senden von Daten etwa 1,5 Stoppbits,
...
die \textbf{optiboot} Seite für alle Testkandidaten gleich war.
...
Den für die Tests verwendeten ATmega1281 habe ich mit einem \textbf{optiboot} Bootloader
...
von \textbf{optiboot} wiederholt. Dabei erzeugt die Einstellung 111111 Baud eine UBRR Einstellung des Hardware UARTs
...
Software-UART Lösung von \textbf{optiboot} versucht. Bei Software-UART zeigte sich schon in der
...
und Software-UART des \textbf{optiboot} mit BAUD\_RATE=32 uneingeschränkt nutzbar.\\

Als weiteren Test mit der Software UART Lösung von \textbf{optiboot} habe ich die BAUD\_RATE=34 Einstellung
...
Der Hintergrund für diese Wahlmöglichkeit ist lediglich ein geringfügig kürzeres \textbf{optiboot} Programm
...
Das liegt wahrscheinlich am schon erwähnten größeren Frequenz-Arbeitsbereich des CP2104 Wandlers mit \textbf{optiboot}
...
und der \textbf{optiboot} Bootloader arbeitete mit den Hardware UART. Bei Verwendung des Software-UARTs
von \textbf{optiboot} wäre das Ergebnis noch besser ausgefallen.
...
werden kann.
Bei einem \textbf{optiboot} mit der Autobaud Funktion kann man ja einfach ausprobieren,
...
Auch für diesen Fall ist die Benutzung der Autobaud Funktion möglich. Da sowohl die Software-UART Lösung
		als auch die Autobaud Funktion mehr Flash Speicher benötigen, wird es immer schwieriger, den \textbf{optiboot}
...
braucht man sich natürlich um die 512 Byte Grenze nicht zu kümmern.
		Die Tests, ob die \textbf{optiboot} Version in 512 Byte passt, sollte man jedenfalls mit \textbf{make} Aufrufen
...
Die Größe der Bootloader-Seite des Zielprozessors wird übrigens bei jedem \textbf{make} Aufruf mit
...
Als ich die Arbeit mit der \textbf{optiboot} Umstellung auf Assembler und Einbau der EEprom-Unterstützung
begonnen hatte, hatte ich keine Ahnung, wie viel hier untersucht und erweitert werden könnte.
Die erste Aufgabe bestand ja darin, einen \textbf{optiboot} zu bauen, der das Laden von EEprom Daten unterstützt
...
sowohl mit dem Hardware-UART als auch mit der Software-UART Lösung von \textbf{optiboot} funktioniert.\\

Die Einstellmöglichkeit des internen RC-Oszillators als Taktgeber für die unterstützten AVR's
wurde untersucht und die Einstellmöglichkeit OSCCAL\_CORR in \textbf{optiboot} integriert.
...
Natürlich läuft \textbf{optiboot} auch dann, wenn mehr als 512 Byte gebraucht werden. Die Grenze war nur meine Zielvorstellung,
...
benutzt wird. Aber hier in \textbf{optiboot} kann diese Funktion benutzt werden und führt zu einer Verringerung
der Fehlers bei der Autobaud Zeitmessung, wenn der Zähler mit /8 Vorteiler benutzt wird.\\

Es tut mir leid, daß die vielen Einstell-Möglichkeiten für \textbf{optiboot} für einen Benutzer verwirrend sind.
Soweit es möglich war, habe ich die Optionen der C-Version von Peter Knight und Bill Westfield übernommen,
damit es Benutzer der Ursprungsversion von \textbf{optiboot} leichter haben.
Sonst hoffe ich aber, daß die Beispiele aus Kapitel \ref{Examples} auf Seite \pageref{Examples}
den ersten Einstieg erleichtern. Außerdem reicht immer ein \textbf{make} Aufruf mit dem gewünschten Zielprozessor als
Parameter aus, um einen lauffähigen \textbf{optiboot} als .hex Datei zu erzeugen. Aus dem Terminal-Protokoll kann man
